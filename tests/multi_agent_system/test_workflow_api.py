"""API 端点测试。

使用 FastAPI TestClient 测试工单 CRUD、知识库上传和统计接口，
通过 mock 外部依赖（workflow、知识库工具）隔离 API 层逻辑。
"""

from contextlib import asynccontextmanager
from datetime import datetime
from types import SimpleNamespace
from unittest.mock import AsyncMock, MagicMock, patch

import pytest
from fastapi import FastAPI
from fastapi.testclient import TestClient
from starlette.middleware.sessions import SessionMiddleware

from src.multi_agent_system.api.routes import router
from src.multi_agent_system.config import Settings
from src.multi_agent_system.core.database import DatabaseManager
from src.multi_agent_system.tools.analytics import AnalyticsTool
from src.multi_agent_system.tools.db_query import DBQueryTool
from tests.conftest import TEST_DATABASE_URL


class _FakeWorkflow:
    """测试用工作流，提供符合 routes._run_workflow 预期的异步迭代接口。"""

    async def astream(self, state):
        yield {
            "process": {
                "status": "processing",
                "processing_result": "测试处理完成",
                "references": ["测试知识库片段"],
            }
        }
        yield {"complete": {"status": "completed"}}


class _FakeTicketIntentAgent:
    """测试用意图理解 Agent，返回确定的结构化工单。"""

    async def extract(self, content: str) -> dict:
        return {
            "title": "系统无法登录",
            "category": "technical",
            "priority": "P1",
            "impact": "部分用户受影响",
            "expectation": "尽快恢复登录",
            "contact": "ops@example.com",
            "occurred_at": "今天 10:15",
            "confidence": 0.92,
            "reason": "测试固定结果",
            "content": (
                "【问题标题】系统无法登录\n"
                "【问题类型】技术支持\n"
                "【紧急程度】P1 高\n"
                "【影响范围】部分用户受影响\n"
                "【期望处理】尽快恢复登录\n"
                "【联系方式】ops@example.com\n"
                f"【原始描述】{content}"
            ),
        }


@pytest.fixture
def app():
    """构建测试用 FastAPI 应用，db_manager 在 lifespan 内创建。"""
    @asynccontextmanager
    async def lifespan(application: FastAPI):
        db_manager = DatabaseManager(database_url=TEST_DATABASE_URL)
        await db_manager.initialize()
        await db_manager.truncate_all()
        db_tool = DBQueryTool(db_manager=db_manager)
        analytics_tool = AnalyticsTool(db_manager=db_manager)
        mock_knowledge_tool = MagicMock()
        application.state.db_manager = db_manager
        application.state.db_tool = db_tool
        application.state.settings = Settings()
        application.state.workflow = _FakeWorkflow()
        application.state.ticket_intent_agent = _FakeTicketIntentAgent()
        application.state.knowledge_tool = mock_knowledge_tool
        application.state.rag_client = None  # 默认跳过双写，单测在用例里覆盖
        application.state.analytics_tool = analytics_tool
        yield
        await db_manager.close()

    application = FastAPI(lifespan=lifespan)
    application.add_middleware(SessionMiddleware, secret_key="test-session-secret-32-chars-or-more")
    application.include_router(router, prefix="/api")
    return application


@pytest.fixture(autouse=True)
def _disable_auth(monkeypatch):
    """reviews/knowledge/settings 路由带 require_role，演示模式视为 admin 放行。"""
    monkeypatch.setenv("AUTH_ENABLED", "false")


@pytest.fixture
def client(app):
    """创建 TestClient 实例，with 触发 lifespan。"""
    with TestClient(app) as c:
        app.state._portal = c.portal
        yield c


class TestTicketAPI:
    """工单 API 测试。"""

    def test_create_ticket(self, client):
        """POST /api/tickets 创建工单。"""
        response = client.post(
            "/api/tickets",
            json={"content": "系统报错", "user_id": "U001"},
        )

        assert response.status_code == 200
        data = response.json()
        assert "ticket_id" in data
        assert data["status"] == "received"

    def test_create_ticket_without_user_id(self, client):
        """POST /api/tickets 不传 user_id 也能成功创建。"""
        response = client.post(
            "/api/tickets",
            json={"content": "咨询问题"},
        )

        assert response.status_code == 200
        assert "ticket_id" in response.json()

    def test_create_ticket_uses_intent_agent(self, client):
        """POST /api/tickets 使用 Agent 理解自然语言并保存结构化字段。"""
        response = client.post(
            "/api/tickets",
            json={"content": "今天 10:15 系统无法登录，请尽快恢复", "user_id": "U007"},
        )

        assert response.status_code == 200
        data = response.json()
        ticket = client.app.state._portal.call(
            client.app.state.db_tool.get_ticket,
            data["ticket_id"],
        )
        assert ticket["category"] == "technical"
        assert ticket["priority"] == "P1"
        assert "【原始描述】今天 10:15 系统无法登录，请尽快恢复" in ticket["content"]

    def test_get_ticket(self, client):
        """GET /api/tickets/{id} 查询工单详情。"""
        ticket_id = "T-QUERY-001"
        client.app.state._portal.call(client.app.state.db_tool.save_ticket, {
            "ticket_id": ticket_id,
            "content": "查询测试",
            "status": "completed",
            "references": ["测试知识库片段"],
        })

        # 通过 API 查询
        response = client.get(f"/api/tickets/{ticket_id}")

        assert response.status_code == 200
        data = response.json()
        assert data["ticket_id"] == ticket_id
        assert data["content"] == "查询测试"
        assert data["references"] == ["测试知识库片段"]

    def test_get_ticket_not_found(self, client):
        """GET /api/tickets/{id} 查询不存在的工单返回 404。"""
        response = client.get("/api/tickets/nonexistent_id")

        assert response.status_code == 404

    def test_list_tickets(self, client):
        """GET /api/tickets 返回工单列表。"""
        # 创建两个工单
        client.post("/api/tickets", json={"content": "工单1"})
        client.post("/api/tickets", json={"content": "工单2"})

        response = client.get("/api/tickets")

        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 2

    def test_list_tickets_with_filter(self, client):
        """GET /api/tickets?status=completed 按状态过滤。"""
        # 创建一个工单并通过 workflow 模拟完成
        create_resp = client.post("/api/tickets", json={"content": "已完成工单"})
        ticket_id = create_resp.json()["ticket_id"]

        # 直接修改数据库中的状态为 completed
        db_tool = client.app.state.db_tool
        client.app.state._portal.call(db_tool.save_ticket, {
            "ticket_id": ticket_id,
            "content": "已完成工单",
            "status": "completed",
            "category": "technical",
            "created_at": datetime.now().isoformat(),
        })

        response = client.get("/api/tickets?status=completed")

        assert response.status_code == 200
        data = response.json()
        assert all(t["status"] == "completed" for t in data)

    def test_list_tickets_with_category_filter(self, client):
        """GET /api/tickets?category=technical 按分类过滤。"""
        db_tool = client.app.state.db_tool
        client.app.state._portal.call(db_tool.save_ticket, {
            "ticket_id": "T020",
            "content": "技术工单",
            "status": "completed",
            "category": "technical",
            "created_at": datetime.now().isoformat(),
        })
        client.app.state._portal.call(db_tool.save_ticket, {
            "ticket_id": "T021",
            "content": "账务工单",
            "status": "completed",
            "category": "billing",
            "created_at": datetime.now().isoformat(),
        })

        response = client.get("/api/tickets?category=technical")

        assert response.status_code == 200
        data = response.json()
        assert all(t["category"] == "technical" for t in data)


class TestKnowledgeAPI:
    """知识库 API 测试（纯代理 rag-service）。"""

    def _mock_rag_client(self):
        """构造 mock rag_client，所有方法返回 AsyncMock。"""
        mock = MagicMock()
        mock.ingest_text = AsyncMock(return_value={
            "doc_id": "abc12345",
            "chunk_count": 3,
            "collection": "ticket_knowledge",
            "action": "created",
        })
        mock.ingest_file = AsyncMock(return_value={
            "doc_id": "pdf99999",
            "chunk_count": 5,
            "collection": "ticket_knowledge",
            "action": "created",
        })
        mock.list_documents = AsyncMock(return_value={
            "total": 1,
            "page": 1,
            "page_size": 50,
            "documents": [
                {
                    "doc_id": "abc12345",
                    "collection": "ticket_knowledge",
                    "source": "admin-upload:测试文档",
                    "category": "technical",
                    "chunk_count": 3,
                    "content_hash": "deadbeef",
                    "extra": {},
                    "ingested_at": "2026-07-05T10:00:00Z",
                }
            ],
        })
        mock.delete_document = AsyncMock(return_value={
            "doc_id": "abc12345",
            "collection": "ticket_knowledge",
            "metadata_removed": 1,
            "points_removed": 3,
        })
        return mock

    def test_upload_knowledge_text_mode(self, client):
        """POST /api/knowledge application/json → rag_client.ingest_text。"""
        client.app.state.rag_client = self._mock_rag_client()

        response = client.post(
            "/api/knowledge",
            json={"title": "工单排查", "content": "步骤 1...", "category": "technical"},
        )

        assert response.status_code == 200, response.text
        data = response.json()
        assert data["status"] == "published"
        assert data["doc_id"] == "abc12345"
        assert data["chunk_count"] == 3
        assert data["action"] == "created"
        # 验证调用了 ingest_text 一次，参数正确
        client.app.state.rag_client.ingest_text.assert_awaited_once()
        kwargs = client.app.state.rag_client.ingest_text.await_args.kwargs
        assert kwargs["collection"] == "ticket_knowledge"
        assert kwargs["text"] == "步骤 1..."
        assert kwargs["category"] == "technical"

    def test_upload_knowledge_file_mode(self, client):
        """POST /api/knowledge multipart → rag_client.ingest_file。"""
        client.app.state.rag_client = self._mock_rag_client()

        response = client.post(
            "/api/knowledge",
            data={"title": "用户手册", "category": "technical"},
            files={"file": ("manual.pdf", b"%PDF-1.4 fake", "application/pdf")},
        )

        assert response.status_code == 200, response.text
        data = response.json()
        assert data["status"] == "published"
        assert data["doc_id"] == "pdf99999"
        # 验证 ingest_file 调用参数
        client.app.state.rag_client.ingest_file.assert_awaited_once()
        kwargs = client.app.state.rag_client.ingest_file.await_args.kwargs
        assert kwargs["filename"] == "manual.pdf"
        assert kwargs["file_bytes"] == b"%PDF-1.4 fake"
        assert kwargs["collection"] == "ticket_knowledge"
        assert kwargs["category"] == "technical"

    def test_upload_knowledge_text_missing_content_returns_400(self, client):
        """text 模式 content 必填。"""
        client.app.state.rag_client = self._mock_rag_client()

        response = client.post(
            "/api/knowledge",
            json={"title": "缺内容"},
        )

        assert response.status_code == 400

    def test_upload_knowledge_no_rag_client_returns_503(self, client):
        """rag_client 未初始化时返回 503。"""
        client.app.state.rag_client = None

        response = client.post(
            "/api/knowledge",
            json={"title": "x", "content": "y"},
        )

        assert response.status_code == 503

    def test_upload_knowledge_rag_service_unavailable_returns_503(self, client):
        """rag-service 不可用时 503。"""
        from src.multi_agent_system.tools.rag_client import RagServiceUnavailable

        mock_rag = MagicMock()
        mock_rag.ingest_text = AsyncMock(side_effect=RagServiceUnavailable("connect refused"))
        client.app.state.rag_client = mock_rag

        response = client.post(
            "/api/knowledge",
            json={"title": "x", "content": "y"},
        )

        assert response.status_code == 503

    def test_list_knowledge(self, client):
        """GET /api/knowledge 透传 rag-service list_documents 响应。"""
        client.app.state.rag_client = self._mock_rag_client()

        response = client.get("/api/knowledge?page=1&page_size=50")

        assert response.status_code == 200, response.text
        data = response.json()
        assert data["total"] == 1
        assert data["page"] == 1
        assert data["page_size"] == 50
        assert data["documents"][0]["doc_id"] == "abc12345"
        # 验证 list_documents 收到 page/page_size
        kwargs = client.app.state.rag_client.list_documents.await_args.kwargs
        assert kwargs["page"] == 1
        assert kwargs["page_size"] == 50
        assert kwargs["collection"] == "ticket_knowledge"

    def test_delete_knowledge(self, client):
        """DELETE /api/knowledge/{doc_id} 调 rag-service delete_document。"""
        client.app.state.rag_client = self._mock_rag_client()

        response = client.delete("/api/knowledge/abc12345")

        assert response.status_code == 200, response.text
        data = response.json()
        assert data["status"] == "ok"
        assert data["doc_id"] == "abc12345"
        client.app.state.rag_client.delete_document.assert_awaited_once()
        kwargs = client.app.state.rag_client.delete_document.await_args.kwargs
        assert kwargs["doc_id"] == "abc12345"
        assert kwargs["collection"] == "ticket_knowledge"

    def test_generate_mock_ticket_uses_llm_with_random_knowledge(self, client):
        """GET /api/tickets/mock-question 基于知识库片段调用 LLM 生成模拟问题。"""
        client.app.state.knowledge_tool.list_documents = MagicMock(
            return_value={
                "documents": [
                    {
                        "id": "doc-1",
                        "title": "云舟科技账号权限手册",
                        "category": "technical",
                        "content": "云舟科技员工更换手机后，CloudID MFA 无法验证时需要人工核身后重置。",
                        "preview": "云舟科技员工更换手机后，CloudID MFA 无法验证时需要人工核身后重置。",
                        "chunk_count": 1,
                        "chunks": [],
                    }
                ],
                "count": 1,
                "next_offset": None,
            }
        )
        llm_response = MagicMock()
        llm_response.choices = [
            MagicMock(
                message=MagicMock(
                    content="我换了手机后 CloudID MFA 一直验证失败，进不了 PeopleHub，请帮我恢复登录。"
                )
            )
        ]

        with patch(
            "src.multi_agent_system.api.routes.CachedLLMClient"
        ) as client_cls:
            client_cls.return_value.chat_completions_create = AsyncMock(
                return_value=llm_response
            )
            response = client.get("/api/tickets/mock-question")

        assert response.status_code == 200
        data = response.json()
        assert data["prompt"] == "我换了手机后 CloudID MFA 一直验证失败，进不了 PeopleHub，请帮我恢复登录。"
        assert data["generation_mode"] == "llm"
        assert data["knowledge_title"] == "云舟科技账号权限手册"
        client_cls.return_value.chat_completions_create.assert_awaited_once()

    def test_generate_mock_ticket_passes_selected_category_to_llm(self, client):
        """GET /api/tickets/mock-question?category=inquiry 会约束 LLM 生成制度流程咨询问题。"""
        client.app.state.knowledge_tool.list_documents = MagicMock(
            return_value={
                "documents": [
                    {
                        "id": "doc-1",
                        "title": "云舟科技制度查询说明",
                        "category": "technical",
                        "content": "云舟科技员工可通过小舟助手或云舟员工门户查询制度入口和审批材料。",
                        "preview": "云舟科技员工可通过小舟助手或云舟员工门户查询制度入口和审批材料。",
                        "chunk_count": 1,
                        "chunks": [],
                    }
                ],
                "count": 1,
                "next_offset": None,
            }
        )
        llm_response = MagicMock()
        llm_response.choices = [
            MagicMock(message=MagicMock(content="我想咨询云舟科技加班餐补制度在哪里看，需要先提交什么审批？"))
        ]

        with patch(
            "src.multi_agent_system.api.routes.CachedLLMClient"
        ) as client_cls:
            client_cls.return_value.chat_completions_create = AsyncMock(
                return_value=llm_response
            )
            response = client.get("/api/tickets/mock-question?category=inquiry")

        assert response.status_code == 200
        data = response.json()
        assert data["category"] == "inquiry"
        call_kwargs = client_cls.return_value.chat_completions_create.await_args.kwargs
        prompt_text = "\n".join(message["content"] for message in call_kwargs["messages"])
        assert "制度流程咨询" in prompt_text
        assert "云舟科技内部员工服务场景" in prompt_text

    def test_generate_mock_ticket_prefers_rag_service_ticket_knowledge(self, client):
        """mock 问题优先从 rag-service ticket_knowledge 检索，不再抽本地旧知识库。"""
        client.app.state.knowledge_tool.list_documents = MagicMock(
            return_value={
                "documents": [
                    {
                        "id": "old-doc",
                        "title": "RSS 内存排查",
                        "category": "technical",
                        "content": "应用服务器 RSS 内存持续上涨，需要定位 OOM。",
                    }
                ],
            }
        )
        client.app.state.rag_client = MagicMock()
        client.app.state.rag_client.retrieve = AsyncMock(
            return_value=(
                [
                    SimpleNamespace(
                        id="chunk-1",
                        content="云舟科技员工登录 CloudID 后 MFA 验证失败，需要人工核身后重置。",
                        metadata={
                            "source": "data/knowledge_base/employee-account-sso-permission.md",
                            "category": "employee-account-sso-permission",
                        },
                    )
                ],
                {},
            )
        )
        llm_response = MagicMock()
        llm_response.choices = [
            MagicMock(message=MagicMock(content="我登录 CloudID 后 MFA 验证失败，无法进入 PeopleHub，请帮忙处理。"))
        ]

        with patch(
            "src.multi_agent_system.api.routes.CachedLLMClient"
        ) as client_cls:
            client_cls.return_value.chat_completions_create = AsyncMock(
                return_value=llm_response
            )
            response = client.get("/api/tickets/mock-question?category=technical")

        assert response.status_code == 200
        data = response.json()
        assert data["prompt"] == "我登录 CloudID 后 MFA 验证失败，无法进入 PeopleHub，请帮忙处理。"
        assert data["knowledge_title"] == "employee-account-sso-permission"
        client.app.state.rag_client.retrieve.assert_awaited_once()
        client.app.state.knowledge_tool.list_documents.assert_not_called()

    def test_generate_mock_ticket_rejects_old_ops_knowledge(self, client):
        """旧 RSS/OOM 运维文档不会再作为随机问题来源。"""
        client.app.state.knowledge_tool.list_documents = MagicMock(
            return_value={
                "documents": [
                    {
                        "id": "old-doc",
                        "title": "应用服务器 RSS 内存定位",
                        "category": "technical",
                        "content": "请问针对应用服务器 RSS 内存随时间持续上涨的情况，有哪些定位步骤？",
                        "preview": "RSS 内存持续上涨，服务响应变慢但未触发 OOM。",
                    }
                ],
                "count": 1,
                "next_offset": None,
            }
        )

        response = client.get("/api/tickets/mock-question?category=technical")

        assert response.status_code == 200
        data = response.json()
        assert data["generation_mode"] == "fallback"
        assert data["knowledge_title"] is None
        assert "RSS" not in data["prompt"]
        assert "OOM" not in data["prompt"]

    def test_generate_mock_ticket_fallback_uses_selected_category_without_knowledge(self, client):
        """知识库为空时，选咨询类型也应返回咨询类兜底问题。"""
        client.app.state.knowledge_tool.list_documents = MagicMock(
            return_value={"documents": [], "count": 0, "next_offset": None}
        )

        response = client.get("/api/tickets/mock-question?category=inquiry")

        assert response.status_code == 200
        data = response.json()
        assert data["generation_mode"] == "fallback"
        assert data["category"] == "inquiry"
        assert any(
            keyword in data["prompt"]
            for keyword in ("咨询", "制度", "入口", "小舟助手", "办公用品", "审批")
        )


class TestAnalyticsAPI:
    """统计 API 测试。"""

    def test_analytics(self, client):
        """GET /api/analytics 返回统计数据。"""
        db_tool = client.app.state.db_tool
        client.app.state._portal.call(db_tool.save_ticket, {
            "ticket_id": "A001",
            "content": "技术问题",
            "category": "technical",
            "priority": "P1",
            "status": "completed",
            "review_score": 0.9,
            "retry_count": 0,
            "created_at": datetime.now().isoformat(),
        })

        response = client.get("/api/analytics")

        assert response.status_code == 200
        data = response.json()
        assert "category_distribution" in data
        assert "priority_distribution" in data
        assert "resolution_stats" in data
        assert "daily_stats" in data

    def test_analytics_empty(self, client):
        """无工单数据时统计接口正常返回。"""
        response = client.get("/api/analytics")

        assert response.status_code == 200
        data = response.json()
        assert data["resolution_stats"]["total"] == 0


class TestSettingsAPI:
    """系统设置 API 测试。"""

    def test_settings_summary(self, client):
        """GET /api/settings 返回前端设置页所需的只读配置摘要。"""
        response = client.get("/api/settings")

        assert response.status_code == 200
        data = response.json()
        assert data["llm_model"]
        assert "llm_api_key_configured" in data
        assert "knowledge_available" in data
        assert "model_routes" in data
        assert "max_react_iterations" in data
