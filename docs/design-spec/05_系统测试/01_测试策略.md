...
> 版本：v2.2
> 日期：2026-07-10
> 状态：对齐企业内部服务台四分区架构，覆盖 rag-service 独立项目、RAG Client 降级、Token 成本统计、决策点埋点修复、Prompt 版本管理
...
- **4 个架构分区**：员工服务端 / 服务台处理端 / 系统运维管理端 / 智能算法与关键技术，每个分区有专属测试范围
- **v1.1 P0 修复验证**：5 个决策点埋点 bug（详见 [13_开发人员工作台设计.md](../01_正式设计/13_开发人员工作台设计.md) 第 11 章）必须回归通过
- **v2.2 对齐能力**：RAG Client 降级、Token 成本统计、Prompt 版本管理、rag-service 独立项目
...
- Token 用量累加、成本统计、Prompt 版本激活/回滚等服务台处理端和系统运维管理端操作行为符合契约
...
| 双项目集成测试 | 主系统 ↔ rag-service 的调用链、降级、超时 |
...
| 测试对象 | 关键场景 |
| --- | --- |
| 服务账号与档案 | 注册、登录、会话失效、联系方式和偏好分类维护 |
| 自助预诊断与智能提单 | 自然语言长描述、预诊断提示、材料清单、TicketIntentAgent 字段提取、空内容校验 |
| 进度协同 | 已存在详情、不存在返回 404、列表分页过滤、WebSocket 节点进度 |
| 员工补充与双向沟通 | `waiting_user_input` 下提交补充消息；非该状态返回 409；空白消息返回 422 |
| 结果验收与申诉升级 | completed 工单 satisfied=false 触发 user_request 审核；satisfied=true 仅记录 |
...
| 知识维护 | 文档上传、入库发布、版本回滚、rag-service 不可用提示 |

### 3.3 系统运维管理端测试（v2.0 新增）
...
| Token 成本控制台 | 日/周/月聚合、热力图、按 model + call_type 分组、成本估算 |
...
主系统通过 `src/multi_agent_system/tools/rag_client.py` 调用 rag-service，集成测试聚焦于**降级、超时**两类异常场景，确保主流程不因 rag-service 问题中断。
...
| 编号 | 场景 | 预期结果 |
| --- | --- | --- |
| TC-TOKEN-101 | 总成本估算 | `estimated_cost_cny` 按模型单价和 token 用量计算 |
| TC-TOKEN-102 | 按模型聚合 | `/tokens` 返回 `by_model` 且总和等于 `total_tokens` |
| TC-TOKEN-103 | 按调用类型聚合 | `/tokens` 返回 `by_call_type` 且覆盖 6 种 call_type |
| TC-TOKEN-104 | 不暴露用户计费口径 | Token API 不返回 per-user 配额字段 |
...
- `GET /api/admin/stats/tokens` 系列返回日、小时、模型、调用类型和成本估算数据。
...
| TC-CLIENT-* | 主系统与 rag-service 集成（降级/超时） | 本文档第 7 章 |
| TC-TRACE-* | 决策点埋点 P0/P1 修复验证 | 本文档第 8 章 |
| TC-PROMPT-* | Prompt 版本管理（创建/激活/回滚/diff） | 本文档第 9 章 |
| TC-TOKEN-* | Token 控制台（累加/聚合/成本估算） | 本文档第 10 章 |
