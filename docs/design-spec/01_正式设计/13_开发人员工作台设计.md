...
> 版本：v2.2
> 日期：2026-07-10
> 状态：归入系统运维管理端，承载流程监控、策略调试和系统健康相关设计
...
开发人员工作台在 v2.2 架构中归入**系统运维管理端**，面向系统管理员和开发运维人员的**系统开发、调试、运维**场景，回答四类问题：
...
> **v2.0 (2026-07-05) 修订**：服务性工单系统不按用户分摊/计费，D-04 仅做系统级总统计（按 model + call_type + 日期分桶）。原 D-07 配额管理已**移除**，12 号文档中关于 per-user 配额/per-user 限额字段/超限降级 的章节均视为历史设计，不再实施。

| 维度 | 决策 |
| --- | --- |
| 路由位置 | DevConsole 左侧导航的第 4 项，独立页（非 Dashboard 子页） |
| 数据来源 | `token_daily_stats` 表（按 model + call_type + date 累加） |
| 统计粒度 | 系统级总用量；不按 user_id 筛选/分摊 |
