...
> 版本：v2.0
> 日期：2026-07-01
> 状态：v2.1 对齐系统级 Token 成本统计；新增 `prompt_versions` / `token_daily_stats` 两张表、`spans.metadata.decision` 子结构说明
...
`token_daily_stats` 是 Token 控制台的数据基础，按系统维度聚合每日 token 用量与估算成本，核心展示维度为 `date`、`model`、`call_type`。`user_id` 仅用于必要时回溯，不作为计费或配额维度。详细 schema 与 UPSERT 逻辑见 [12_Token成本控制台设计.md](./12_Token成本控制台设计.md) 第 3 节。
