...
> v2.0 修订：本文档已对齐 4 大模块架构与双项目架构。第 3 节状态流转图新增 RAG 调用节点（`tools/rag_client.py` HTTP 调用 rag-service）与降级虚线；第 4 节前新增「RAG 调用策略」小节；第 5 节新增「RAG 调用方式变化」说明。
...
```mermaid
flowchart TB
    Start([提交工单]) --> Intent["TicketIntentAgent 结构化自然语言工单"]
