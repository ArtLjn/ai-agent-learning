...
> 版本：v2.2
> 日期：2026-07-10
> 状态：页面清单对齐企业内部服务台四分区架构，系统运维管理端包含 D-01 ~ D-06，Token 控制台仅做系统级成本统计
...
前端作为系统演示和操作入口，需要让企业内部员工能够注册登录、维护服务档案、提交工单、采集材料、查看处理进度、补充信息、验收处理结果和申诉升级；让服务台处理人员能够上传知识内容、处理人工审核队列并查看运营分析；让系统运维管理人员能够观察状态机、Trace、Prompt、RAG、Token 和系统健康。界面定位为轻量管理端，不做复杂客服坐席系统。

v2.2 起，前端对外按 4 个架构分区解释页面：员工服务端、服务台处理端、系统运维管理端、智能算法与关键技术。实现上继续保留 U/A/D/I 编号，覆盖 [系统模块架构清单](../assets/system-module-architecture-v2-ascii.md) 中定义的 29 个功能。

页面整体仍保持轻量级——所有页面共享同一 React 应用（`web/src/`），通过侧边栏导航切换；不引入微前端或独立部署。

## 2. 页面结构（四个架构分区映射到页面导航）

### 2.1 员工服务端页面（覆盖 U-01 ~ U-08）
...
### 2.2 服务台处理端 / 系统运维管理端页面（覆盖 A-01 ~ A-07）
...
- [00_预设计/02_系统功能与总体架构.md](../00_预设计/02_系统功能与总体架构.md) — v2.2 四个架构分区总体架构
