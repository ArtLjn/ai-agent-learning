...
> 版本：v2.0
> 日期：2026-07-01
> 状态：页面清单严格对齐 30 功能（U-/A-/D-/I- 编号），新增 Register / Profile / UserManagement / SystemConfig / AuditLog / AgentCallStats / ServiceHealth 7 个页面
> 关联：[assets/system-module-architecture-v2-ascii.md](../assets/system-module-architecture-v2-ascii.md)（30 功能清单权威源） · [13_开发人员工作台设计.md](./13_开发人员工作台设计.md) · [12_Token成本控制台设计.md](./12_Token成本控制台设计.md) · [00_预设计/02_系统功能与总体架构.md](../00_预设计/02_系统功能与总体架构.md)

## 1. 设计目标

前端作为系统演示和操作入口，需要让用户能够注册登录、提交工单、查看处理进度、检查处理结果、维护个人资料、上传知识库内容、处理人工审核队列，并观察 Agent 执行状态。界面定位为轻量管理端，不做复杂客服坐席系统。

v2.0 起，前端按 4 大角色模块组织：用户模块、管理员模块、开发人员模块、智能算法模块。每个模块对应一组页面，覆盖 [assets/system-module-architecture-v2-ascii.md](../assets/system-module-architecture-v2-ascii.md) 中定义的 30 个功能。

页面整体仍保持轻量级——所有页面共享同一 React 应用（`web/src/`），通过侧边栏导航切换；不引入微前端或独立部署。

## 2. 页面结构（按 4 大模块组织，对齐 30 功能）

### 2.1 用户模块页面（覆盖 U-01 ~ U-08）

| 页面 | 文件 | 覆盖功能 | 说明 |
| --- | --- | --- | --- |
| 注册 | `Register.tsx`（v2.0 新增） | U-02 | 自助注册账户，注册成功自动登录 |
| 登录 | `Login.tsx` | U-01 | 用户名密码登录，成功后进入系统 |
| 个人资料 | `Profile.tsx`（v2.0 新增） | U-03 / U-04 | 用户信息管理（昵称/联系方式/偏好分类）+ 修改密码 |
| 工单列表 | `Tickets.tsx` | U-05 / U-06 | 工单列表与筛选；含 AI 创建工单入口（`AgentTicketComposer`） |
| 工单详情 | `TicketDetail.tsx` | U-06 / U-07 / U-08 | 工单详情、进度、结果、trace；`waiting_user_input` 状态下显示补充输入框（U-07）；`completed` 状态下显示满意度反馈区（U-08） |
| 知识库（用户可见） | `Knowledge.tsx` | （只读辅助） | 只读视图：用户可浏览公开知识条目，不能上传/编辑（写权限归管理员） |

#### 2.1.1 Register.tsx（U-02 用户注册，v2.0 新增）
...
#### 2.1.2 Profile.tsx（U-03 / U-04，v2.0 新增）
...
| 用户管理 | `UserManagement.tsx`（v2.0 新增） | A-04 | 用户列表、封禁/解封、Token 配额覆写 |
...
- 表格列：username、nickname、status、vip_level、token_monthly_limit（NULL 显示「默认」）、token_weekly_limit、created_at
- 操作列：
  - 封禁/解封按钮（切换 status，调用 `PATCH /api/admin/users/{user_id}`）
  - 配额调整：点击弹出 Sheet，编辑 `token_monthly_limit` / `token_weekly_limit`，留空表示走默认
- 二次确认：所有写操作弹出确认对话框
...
| 默认配额 | `config.token_quota.monthly_limit` / `weekly_limit` |
...
- 顶部筛选栏：`admin_id`、`action`（审核决策/封禁/解封/配额调整/知识库删除/版本回滚/Prompt 激活）、`target_type`、时间区间
...
### 2.3 开发人员模块页面（覆盖 D-01 ~ D-07，v2.0 新增）

开发人员工作台是一个独立页面族，主页 `DevConsole.tsx` 提供左侧导航，切换 7 个子模块：

| 页面 | 文件 | 覆盖功能 | 说明 |
| --- | --- | --- | --- |
| DevConsole 主页 | `DevConsole.tsx` | （外壳） | 开发人员工作台外壳，左侧 7 项子模块导航，右侧渲染当前选中子页 |
...
| Token 控制台 | `dev/TokenDashboard/` | D-04 / D-07 | 日/周/月维度 Token 用量统计、按 model + call_type 分组、配额管理（复用 explore/farm-manager 架构，详见 [12](./12_Token成本控制台设计.md)） |
...
| `/dev/tokens` | Token 成本控制台（D-04 / D-07） |
...
工单列表页提供 `AgentTicketComposer` 组件，用户不需要手工填写分类、优先级等传统表单，只需要输入自然语言描述。后端 `TicketIntentAgent`（I-01）会自动提取问题标题、分类、优先级、影响范围和联系方式。
...
### 3.1 用户提单主流程（覆盖 U-02 / U-05 / U-06 / U-07 / U-08）
...
flowchart LR
    ...
    M[waiting_user_input 状态] --> N[用户补充信息]
    N --> F
```
...
| Token 控制台（D-04 / D-07） | `/dev/tokens` | `GET /api/admin/stats/tokens` |
...
- `UserProfile`：用户可编辑资料（v2.0 新增，U-03）
...
- `TicketFeedback`：满意度反馈（v2.0 新增，U-08）
...
- `RegisterRequest`：注册请求（v2.0 新增，U-02）
...
- `ChangePasswordRequest`：修改密码请求（v2.0 新增，U-04）
...
- `UserQuota`：用户配额覆写状态（v2.0 新增，对应 `users.token_monthly_limit` / `token_weekly_limit`）
...
- `SystemConfig`：系统配置摘要（v2.0 新增，A-06）
  ...
  interface SystemConfig {
    ...
    default_quota: { monthly_limit: number; weekly_limit: number };
  }
  ```
...
- `AuditLogEntry`：操作日志条目（v2.0 新增，A-07）
  ...
  interface AuditLogEntry {
    ...
    action: string; // 'review_decision' | 'user_ban' | 'user_unban' | 'quota_adjust' | 'knowledge_delete' | 'knowledge_rollback' | 'prompt_activate'
...
- `TokenStats`、`TokenDailyStats`、`TokenHourlyStats`、`QuotaStatus`：Token 控制台类型（新增，D-04，详见 [12](./12_Token成本控制台设计.md)）
...
- 能直观看到工单从提交到完成的变化（U-05 ~ U-08）。
- 能让用户自助注册与维护个人资料（U-02 ~ U-04）。
- 能展示多 Agent 每个节点的执行结果（I-01 ~ I-08）。
- 能上传知识文档体现 RAG 能力（A-02 / A-03）。
- 能通过审核工作台展示人机协同闭环（A-01）。
- 能通过开发人员工作台展示 Trace 决策、Prompt 版本、RAG 调试、Token 成本、Agent 调用统计、服务健康（D-01 ~ D-07，v2.0 新增）。
...
- [02_产品需求/01_核心功能需求.md](../02_产品需求/01_核心功能需求.md) — 30 功能的验收标准
- [06_可观测与执行追踪设计.md](./06_可观测与执行追踪设计.md) — Trace/Span 基础数据契约
- [09_人工审核工作台设计.md](./09_人工审核工作台设计.md) — 审核工作台详细设计（A-01）
- [12_Token成本控制台设计.md](./12_Token成本控制台设计.md) — Token 控制台详细设计（D-04 / D-07）
- [13_开发人员工作台设计.md](./13_开发人员工作台设计.md) — 开发人员工作台子模块详细设计（D-01 ~ D-03）
