...
> 版本：v2.0
> 日期：2026-07-01
> 状态：页面清单严格对齐 29 功能（U-/A-/D-/I- 编号），新增 Register / Profile / UserManagement / SystemConfig / AuditLog / AgentCallStats / ServiceHealth 7 个页面
> 关联：[assets/system-module-architecture-v2-ascii.md](../assets/system-module-architecture-v2-ascii.md)（29 功能清单权威源） · [13_开发人员工作台设计.md](./13_开发人员工作台设计.md) · [12_Token成本控制台设计.md](./12_Token成本控制台设计.md) · [00_预设计/02_系统功能与总体架构.md](../00_预设计/02_系统功能与总体架构.md)

## 1. 设计目标

前端作为系统演示和操作入口，需要让用户能够注册登录、提交工单、查看处理进度、检查处理结果、维护个人资料、上传知识库内容、处理人工审核队列，并观察 Agent 执行状态。界面定位为轻量管理端，不做复杂客服坐席系统。

v2.0 起，前端按 4 大角色模块组织：用户模块、管理员模块、开发人员模块、智能算法模块。每个模块对应一组页面，覆盖 [assets/system-module-architecture-v2-ascii.md](../assets/system-module-architecture-v2-ascii.md) 中定义的 29 个功能。

页面整体仍保持轻量级——所有页面共享同一 React 应用（`web/src/`），通过侧边栏导航切换；不引入微前端或独立部署。

## 2. 页面结构（按 4 大模块组织，对齐 29 功能）
