...
> v2.0 修订：本文档已对齐 4 大模块架构。**文件名保留不改**（沿用历史引用），但本文档聚焦 **智能算法模块**（4 大模块中的算法层）。第 1 节扩展为说明智能算法模块在 4 大模块中的协作定位；新增「与 rag-service 的集成」「决策追踪结构化」两章；第 4 节工具层由内部 Qdrant 访问改为 HTTP 调用 rag-service。

## 1. 设计目标

> v2.0 模块归属：本文档对应 4 大模块中的**智能算法模块**。智能算法模块负责 5 个 Agent（Intent / Classifier / ReActProcessor / Reviewer / Coordinator）+ LangGraph 编排 + 工具层 + 决策追踪，与另外三大模块的协作关系如下表。
...
### 1.2 智能算法模块在 4 大模块中的定位（v2.0 新增）

v2.0 主系统按「用户 / 管理员 / 开发人员 / 智能算法」4 大角色模块切分，智能算法模块作为算法底座，与其它三个模块的协作关系如下：

| 协作对象 | 协作方式 | 文档链接 |
| --- | --- | --- |
| 用户模块 | 接收用户模块 `create_ticket` 接口转交的原始工单，由 TicketIntentAgent 完成结构化后回写工单字段；处理完成后通过 WebSocket 向用户模块推送节点进度与结果 | [02_工单处理流程设计.md](./02_工单处理流程设计.md) |
| 管理员模块 | 向 CoordinatorAgent 的 `suggest_decision` 提供 AI 建议，被采纳后写入人工审核表；处理失败时通过 `escalate` 节点转入管理员审核工作台 | [09_人工审核工作台设计.md](./09_人工审核工作台设计.md) |
| 开发人员模块 | 决策点结构化埋点写入 `spans.metadata.decision`，供 Trace 决策树渲染；Prompt 模板从 `prompt_versions` 表加载；Token 用量回写到 trace | [13_开发人员工作台设计.md](./13_开发人员工作台设计.md) |

智能算法模块本身不再承担用户鉴权、知识库 CRUD、Trace 可视化等职责，这些已被明确切分到对应模块。模块详细职责划分见 [02_系统功能与总体架构.md](../00_预设计/02_系统功能与总体架构.md)。
