...
| RAG 模块定位不清 | `src/rag_systems/` 曾同时包含个人知识库、论文阅读助手、文档处理工作流等学习内容 | 已将论文阅读助手和文档处理工作流归档；v2.0 进一步把 RAG 检索能力抽离为独立 rag-service |
...
| `ai-agent-learning/src/rag_systems/` | 个人知识库和 RAG 辅助能力（学习用） | 部分属于 | 只保留与知识库能力相关的模块 |
...
| 配额服务 | `services/quota_service.py` | 待建 | per-user 月/周配额检查（`check_user_quota`）；从 `core/cached_client.py` 的 LLM 调用入口拦截，详见 [12](./12_Token成本控制台设计.md) 第 4.1 节 |
...
- v2.0 新增模块测试：`tests/api/test_admin_trace.py` / `test_admin_stats.py` / `test_admin_prompts.py` / `test_admin_rag.py` / `tests/core/test_token_stats.py` / `tests/services/test_quota_service.py`（按需新增）。
...
- 主系统新增 `tools/rag_client.py`、`core/token_stats.py`、`services/quota_service.py`、`api/admin_*.py` 系列路由。
...
- [12_Token成本控制台设计.md](./12_Token成本控制台设计.md) — `core/token_stats.py` / `services/quota_service.py` / `api/admin_stats.py` 的详细设计
