...
## 1. 功能概览（v2.0 4 大角色模块，共 29 个功能）

v2.0 起，主系统按"角色 + 算法"拆分为 4 大模块共 **29 个功能**。每个模块对应用户角色或核心算法能力，回应答辩反馈"模块要按角色分层、工作量不够"。完整功能编号清单（U-/A-/D-/I- 前缀）见 [assets/system-module-architecture-v2-ascii.md](../assets/system-module-architecture-v2-ascii.md)。

| 模块 | 功能数 | 主要功能 | 对应源码 |
| --- | --- | --- | --- |
| **用户模块** | 8 | 身份认证与会话、服务身份初始化、联系信息维护、偏好分类设置、自然语言问题描述、服务请求跟踪、补充信息交互、结果反馈与升级 | `api/auth_routes.py`、`api/routes.py`、`agents/ticket_intent.py`、`web/src/pages/Login.tsx`、`Tickets.tsx`、`TicketDetail.tsx`、`Profile.tsx`（待建） |
...
| **开发人员模块** | 6 | Trace 决策树、Prompt 版本对比、RAG 检索调试器、Token 成本控制台、Agent 调用统计、服务健康检查 | `api/admin_trace.py`（待建）、`api/admin_stats.py`（待建）、`api/admin_prompts.py`（待建）、`web/src/pages/dev/`（待建，复用 explore/TokenDashboard） |
...
- **用户模块**：自然语言问题描述 / 工单查询 / 补充信息交互 / 状态订阅 / 结果反馈与升级
...
- **自然语言问题描述**：用户输入自然语言描述，`TicketIntentAgent` 自动提取标题/分类/优先级/影响范围/联系方式。
- **工单查询**：列表筛选、详情查看、状态追踪。
- **补充信息交互**：审核员请求补充后，用户在 `waiting_user_input` 状态提交订单号/支付流水等。
- **状态订阅**：通过 WebSocket 实时接收工单节点进度。
- **结果反馈与升级**：工单完成后用户提交满意/不满意评分。
...
- **用户管理**：用户列表、封禁、解封、角色调整（admin / developer）。
